...
def get_list(
    ...
    joins = [
        (ProductCategory, and_(ProductCategory.id == Product.category_id)),
    ]
    options = [contains_eager("product_categories")]
    return product_service.find_paginated(
        {
...
def create(request: Request, body: ProductCreate, db: Session = Depends(get_database)):
...
def updated_by_code(
    code: int, update_body: ProductCreate, db: Session = Depends(get_database)
):
